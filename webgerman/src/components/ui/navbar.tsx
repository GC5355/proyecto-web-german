  ...
  Agustin Pardo Motz
      ...
      <ul className="hidden col-start-5  col-span-3  lg:flex content-center space-x-10 ml-11 items-center justify-center pt-2">
        {nav_links.map((link, i) => (
          <Link href={link.href} key={i} className=' px-3 font-semibold font-montserrat tracking-wide text-neutral-950  cursor-pointer pb-1.5 transition-all hover:font-bold select-none'>
            <div
              onClick={() => handleLinkClick(link.title)}
              className={`px-3 font-semibold tracking-wide text-neutral-950 pb-1.5 transition-all hover:font-bold ${selectedEnlace === link.title ? 'underline ' : ''
                }`}
            >
              {link.title}
            </div>
